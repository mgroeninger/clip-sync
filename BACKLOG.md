...
| [TEMP-gap-fixture-corpus-plan.md](docs/TEMP-gap-fixture-corpus-plan.md) | Curated per-gap-type fixture corpus replacing the ephemeral `gap-files/` dependency — **all phases done (0–5)**: 11 committed fixtures (9 real + 2 synthetic; `unfillable` is plan-time, not representable), loader, PR-tier per-type test, self-hosting golden; re-anchor retired. Ready to archive |
