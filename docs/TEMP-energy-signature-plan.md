...
> **Corpus plan:** [energy-corpus-plan.md](archive/energy-corpus-plan.md) — long synthetic F1/F2 @ production geometry, mode matrix, optional PD/CC profile → regenerate (no copyrighted media in repo).

- [x] Corpus / manual pass: mode matrix recorded with vocabulary tags; acceptance **EC-1–EC-6** in [energy-corpus-plan.md](archive/energy-corpus-plan.md) (**EC-6 met** via `build_f4_decoy_production` — energy resolves a 7 s-off nominal where bool stays at the decoy).
