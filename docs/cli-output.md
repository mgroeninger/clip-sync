...
**Exception — post-scan selection/config errors (human format only).** When `--only-gaps` / `--skip-gaps` (or TOML peers) fail validation after the gap table exists (bad gap number, empty selection, etc.), the human format still prints the gap table on stdout so the operator can copy `#` values, then prints the error on stderr and exits **2**. Under `--format json`, stdout stays empty (no success-shaped document) — matching the failure row above.
...
| Scan | — | `Scanning video A for gaps...`; after scan: `Gap scan: N silent run(s) ≥…ms — …`; when some gaps are not repairable, `Gap fill: R of N repairable (M skipped — …)`; when `--only-gaps` / `--skip-gaps` narrows the plan, `Gap filter: selected K of N detected gaps (only-gaps: …)` / `(skip-gaps: …)` (omitted if the list names every gap **and** no straddler needs naming; scan-only validates tokens but does not emit this line). When a `START..END` token leaves a straddling gap unselected under `--only-gaps`, the same line appends `; gap #N overlaps the window but is not fully inside it — not selected` (omitted if another token selected that gap). Under `--skip-gaps`, the polarity is `; gap #N overlaps the skip window but is not fully inside it — still selected` |
...
Normative detail for patch outcomes; user-facing summary in [README.md](../README.md) § Gap patching pipeline. **Classifying gaps and choosing settings:** [gap-repair-guide.md](gap-repair-guide.md). **Flag matrix, extension semantics, performance:** [gap-fill-modes.md](gap-fill-modes.md).
