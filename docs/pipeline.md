...
**Fill-level check (`measure_fill_level`, on by default).** Between steps 4 and 5 — final patches chosen, A not yet mutated — each fill's loudest 100 ms bin is measured against the A shoulders either side of its gap and recorded on the outcome as `fill_level`. It is **report-only**: no threshold, no veto. It exists because a fill placed into quiet A at a level the surrounding program never reaches is audible as damage even when every seam score is clean, and because a veto here would trade that for unrepaired holes — so the number is collected and calibrated first. The fill measured is exactly what the splice writes (`gained_fill` — gain applied, clamped, cut to the destination), and a shoulder with less than half its width of room is declined rather than measured from a sliver. Two rows must not be read naively: `reference_at_floor` means every shoulder was itself digital silence (a neighbouring dropout), and the seam crossfade is ignored. **Read `edge_delta_db`, not `peak_delta_db`** — ear labels over the 39-pair corpus (2026-08-06) refuted the peak as an ordering, and the peak is kept only as the interior half of the pair (the distance between them is what says a transient sits inside the fill). See [json-output.md](json-output.md) § FillLevelCheck. Corpus sweep over a pair manifest: `scripts/measure/measure-patch-outcomes.ps1` (`--patch-only` JSON + `fill-level-rollup.csv`, sorted by `edge_delta_db`, floored references held out) — it captures every per-gap patch outcome, not just this one, and `align_adjustment_secs` in its reports is where the placement defect in BACKLOG.md § "Fill placement is accepted without checking local registration" was found. The measurement needs the splice, so `--repair-preview` cannot produce it and `--patch-only` is the cheapest run that can. **No threshold is proposed, and the corpus cannot supply one.** A distribution gives the cost of a candidate cut, never its correctness — that needs ear labels. The ten collected so far say two things a future calibration must not re-derive from scratch: a cut on `edge_delta_db` alone is already refuted (a correct +9.83 dB restoration of a gunshot into conversation would fail it), and the axis that conditioned it was registration quality (`pre_correlation`), not level. Ear-check candidates with `measure-gap-fingerprints.ps1 -ScanArgs "--gap-listen"` narrowed by `--fingerprint-gap`. Finally, this pass measures **level, not placement**: two of those ten clips carried damage it cannot see at all (a fill duplicating the shoulder, and one running seconds past the gap), both at unremarkable deltas — a clean `fill_level` is not a clean repair.
