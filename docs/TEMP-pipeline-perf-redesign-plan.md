...
- **Real-bracket carry (8g.5) = dedicated `real_brackets` param.** `spec_to_fingerprint_summary(spec, sr, ch,
  x, real_brackets: Option<Vec<BracketInfo>>)`; `Some` ⇒ emit the real rows, `None` ⇒ `synth_brackets`
  fallback. Keeps bracket **counts** (a decision axis, D) first-class rather than conflating them into the
  X-set. *(Chosen over carrying brackets inside `FingerprintXSet`.)*
...
| **8g.4b** | **Flip the dump — only after 8g.4a is green on licensed media (both modes).** `dump_gap_fingerprints` / `characterize_gaps_with_gate` → call `characterize_gaps_from_decode`; **stop running the per-bracket oracle unconditionally** (the §1.3 dominant cost — a **diagnostic-dump** speedup, not production); delete the old inline oracle path. Clean up stale comments (`patch_audio` `characterize_all_regions` rustdoc, `fixtures/fingerprint_corpus_fixtures.rs` "at 8g.3", orphaned rustdoc above `RegionMeasurements`) as part of the flip. | oracle-unconditional; **perf win (diagnostic dump)** | 8g.4a differential green on licensed media (lean + `--fingerprint-diagnostics`) + re-anchor; then flip |
| **8g.5** | **Diagnostics fidelity.** On `--fingerprint-diagnostics`, `measure_region_tags` also computes the X-set (`seam_probe`/`wide_envelope`/`b_levels`/diagnostic `lag`) **+ the real `Vec<BracketInfo>`** (the per-bracket enumeration moves *here*, run on demand); `spec_to_fingerprint_summary` gains the `real_brackets` bypass. Until then, harness `project_corpus` carries only `b_levels` in the X-set (full diagnostics round-trip not exercised pre-8g.5). | fidelity constraint (troubleshooting survives) | Decode differential (`--fingerprint-diagnostics`) equal **incl. X + real brackets** |
...
(measure first; the per-bracket-oracle gating that 8g just delivered may already be the bigger win).
