...
just decision axes; the **perf win** (gating the per-bracket oracle — which the from-decode path still runs
unconditionally, `gap_fingerprint.rs:2688`) is **decoupled into the redefined 8g.5**, since it needs a cheap
lean tier+counts path first.
...
NEXT: 8g.5 (perf: gate the per-bracket oracle — needs the counts-reclassification decision, see the row), then
8g.6 (delete residual oracle + `--validate-from-decode`, C5/C3-dump), §3 step 8 (Hoists). Work through 8g.4a
...
| **8g.5** | **Perf: gate the per-bracket oracle behind diagnostics (the actual §1.3 dominant-cost win — REDEFINED 2026-07-11).** The from-decode path still enumerates + scores **every** bracket unconditionally (`gap_fingerprint.rs:2688`) — that per-bracket `oracle_score_fit_candidate` sweep is the §1.3 dominant cost. The goal: move the full enumeration/scoring behind `--fingerprint-diagnostics`, so the **lean** dump doesn't pay `N_brackets × O(N·radius)`. **This is NOT a mechanical `if diagnostics { … }` wrap — it is blocked by an invariant, and resolving that is design work to be scoped first (hence decoupled from the flip):** <br>• **The blocker.** `brackets_total` **and** `brackets_passing` are frozen as **Tier-1 bit-exact** golden axes (`golden_baseline.rs:103`, `tier1!` 188–189). An *exact* `brackets_passing` can only come from scoring every bracket — the very cost we want to cut. So "gate the oracle off in lean" and "lean stays byte-identical on `GoldenBaseline`" are contradictory **as currently tiered**. (The doc's earlier "or shared gate cache" lever doesn't apply — a `--gap-fingerprints` run has no concurrent production gate to share with.) <br>• **Observation that opens a path.** No consumer reads the *exact* count — every decision uses a threshold: `bracket_exhausted = total>0 && passing==0` (`gap_fingerprint_corpus.rs:565`), skip filter `passing==0` (1723), patch `passing>0` (560/1812); `brackets_total` is cheap anyway (`list_feasible_anchor_brackets` = geometry, no scoring). So the decisions need only **"≥1 passing?"**, which is **short-circuitable** (stop at first passing bracket). Exact `brackets_passing` is diagnostic richness that was promoted to a Tier-1 axis it may not deserve. <br>• **EXPLORE — reclassification (candidate approach, NOT yet decided; refined by the 8g.5 review 2026-07-11).** Demote **only exact `brackets_passing`** from Tier-1 to diagnostic (exact on `--fingerprint-diagnostics`); **KEEP `brackets_total` Tier-1** — it's cheap (`list_feasible_anchor_brackets` geometry, no scoring) and distinguishes no-placement (`0`) from exhausted (`>0`). Lean golden asserts `{ tier, brackets_total, bracket_exhausted }` (already all Tier-1) — which fully encode the decision model — and simply stops asserting exact `brackets_passing`. Then lean short-circuits at the first passing bracket; diagnostics keeps the full enumeration + real rows (8g.4b). <br>• **PERF EXPECTATION — partial win, measured before committing.** Short-circuit only helps **patch-tier** gaps (stop at first `Ok`); **bracket-exhausted skips must still score every bracket to prove exhaustion (no win)**. On the 7-pair licensed media the split is **20 patch / 49 skip = 29% patch** — so the corpus-wide win is modest, and even patch gaps save only the brackets *after* the first pass. This tempers "the actual dominant-cost win": the **spike must measure wall-clock** (7-pair media + synth) before committing; keeping exact counts and accepting no lean win stays a live alternative if the measured win is small. <br>• **Short-circuit safety — RESOLVED (was an open question).** Brackets sort by `move_frames` ascending (`gap_anchor_seam.rs:315`), throat (zero-move) first. Tier uses `any_ok` (not *which* bracket won) ⇒ first-pass ≡ full-scan for tier; cell uses `seam_local`/donor axes, not bracket identity; residual uses throat placement (existing `oracle_throat_structure_frame` fallback covers throat-fails-but-later-passes). So short-circuit does **not** change tier or cell. <br>• **GOLDEN REFRESH — scoped (was the open question).** Concrete: remove `tier1!(brackets_passing)` from `golden_baseline.rs` (this drops it from the diff entirely — no mode-aware differential needed; diagnostics exact-count fidelity is already covered by `from_decode_matches_oracle_full_fidelity_brackets_and_xset`, which compares bracket rows via JSON, not `GoldenBaseline`); regenerate `crates/clip-sync-repair-harness/golden/re-anchor-dual-fit-on-nominal.golden.json`; update the C4 projection test to stop asserting exact `brackets_passing` (keep `bracket_exhausted`/cell identity); update the §4.1 table + schema string. Downstream corpus reports (`dualfit_viability_text`, `passing==0` filters) keep working on thresholds; they lose "3 of 7 passed" granularity **only on lean** corpora (diagnostics retains exact). <br>• **IMPL GOTCHA.** After short-circuit, `brackets.len()` is the *scored* count, not the feasible total — so the Tier-1 `brackets_total` must be carried separately from `list_feasible_anchor_brackets` (before the loop), and lean `brackets_passing` export becomes a sentinel (`1` if any pass, `0` if exhausted). <br>Diagnostics-on X-set is **already faithful (0/58)**; real brackets are carried at 8g.4b — so 8g.5 is purely the lean-path perf + the reclassification decision. | perf (diagnostic-dump → lean-dump); §1.3 dominant cost | Spike behind a flag; before/after `golden_baseline` diff shows only `brackets_passing` drops (tier/`bracket_exhausted`/`brackets_total` unchanged); decode differential still equal both modes; **measured** lean wall-clock improvement (or decide not worth it) |
