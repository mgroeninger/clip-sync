...
| **8g.3a** | **Extract `compute_region_measurements` (byte-identical refactor).** Pull `characterize_gaps_with_gate`'s per-gap overlay loop (the `fp.brackets`/`outcome`/`baseline_lag`/`donor_interior`/`donor_interior_nominal`/`splice_dualfit` + diagnostics `seam_probe`/`b_levels`/`wide_envelope`/`lag` assignments, `gap_fingerprint.rs` ~2582–2870) into `compute_region_measurements(...) -> RegionMeasurements` (raw primitive outputs); the loop assigns `fp` fields **from** the struct. Output must stay identical. Delicate (~290 lines; shared `b_mono`/`b_ch`/geometry locals). | — | 8g.1 decode differential still green (oracle unchanged) + diagnostics-toggle test + media differential; no `GapFingerprint` byte change |
