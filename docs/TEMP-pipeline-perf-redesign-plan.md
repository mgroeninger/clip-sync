...
| **8g.3b** | **From-decode tags (the crux).** `tags_from_measurements(&RegionMeasurements) -> GapRepairTags`; a from-decode corpus generator runs `characterize_all_regions` + `compute_region_measurements` → tags → project. **Shadow-assert:** from-decode projection reproduces `tags_from_fingerprint(old_oracle_fp)`'s `golden_baseline` on the same input (both lean and `--fingerprint-diagnostics`). `structure`/`seams` omitted to match `skip_baseline_placement`. | (e) *from-decode half* | Decode differential (both modes): from-decode projection == old oracle `golden_baseline` |
