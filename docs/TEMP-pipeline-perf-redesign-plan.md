...
| **8 (Fingerprint-unification)** | `characterize_gaps_with_gate` → shared characterize + X export only. **Clears (resolution index):** (d) pin `skip_cell_from_tags` `seam_local`; (e) populate `tags_ctx` D/R payload; (f) fix SilenceSplice inert geometry (#1); make skips real `Skip`-verdict specs. **Decomposed into 8a–8g — see the Fingerprint-unification sub-step ledger below.** | §4.7 C3; no second gate oracle in dump path |
...
| **8g** | Flip `dump_gap_fingerprints` → `characterize_gaps_with_gate` to call the shared `characterize_all_regions` (**new — only per-region `characterize_region` exists today**, `patch_audio.rs:2156`) + **compute the R/X tags from decode directly** (the shared primitives `lag_at_placement` / `donor_interior_at` / `splice_dualfit_at` / bracket oracle / `level_profile`, **order-independent** per §2.5.7 #5 H2 — absorbed from 8f) + `spec_to_fingerprint_summary`. **"Delete the second gate oracle" (`gap_fingerprint.rs:2132–2176`) = stop running it UNCONDITIONALLY, not remove the capability.** **Fidelity constraint — lean-by-default, full-on-diagnostics (troubleshooting must survive the flip):** the projection is lossy on X/brackets by design (§8f), so a straight flip would gut the diagnostic corpus (`seam_probe`/`wide_envelope`/diagnostic `lag` = the "why a seam is dead"; real per-bracket `failure_stage`/`move_frames`/times = which bracket failed at which stage). Therefore: **default** dump = D/R + decision axes only (lean, fast); **`--fingerprint-diagnostics`** dump = the full `FingerprintXSet` **and the real `Vec<BracketInfo>`** carried through the projection (the per-bracket oracle moves *into* the diagnostics overlay — run on demand, not deleted), so the on-diagnostics corpus is as rich as today. `spec_to_fingerprint_summary` currently **synthesizes** brackets (counts/best/closest only); 8g must add a path that carries the real per-bracket rows when diagnostics are on. Note `outcome.skip_reason` changes `"gate skipped"` → the real reason tag (a corpus change; arguably better for troubleshooting). **Validation — the 8f corpus-differential is necessary but NOT sufficient (learned 2026-07-09 running it on real media).** The 8f `gap_repair_spec_diff` is corpus→tags→project (no decode) — it proves the projection is decision-faithful (**now validated on the in-repo re-anchor 69 gaps AND a user 5.1 licensed run, 35 gaps, both green**). But 8g's new risk is that **from-decode tags == the oracle's** — which the corpus-only differential can't see (it starts from the oracle's own fingerprint). 8g therefore needs a **decode differential**: run the new from-decode characterize on the *same media* the old oracle scanned, project, and diff `golden_baseline` against the old corpus. **This requires media** (the licensed run is the vehicle; the corpus JSON alone won't do). Run it in **both modes** — lean (default) *and* `--fingerprint-diagnostics` (the fidelity path) — since a plain run only exercises the lean path. `structure`/`seams` stay `None` under the current `skip_baseline_placement` summary, so from-decode must match (omit them) — the differential guards it. C5 makes the corpus-differential live. | §4.7 **C3**, **C5**, §3 step 7 | Decode-differential green on ≥1 real media run in **both** lean and `--fingerprint-diagnostics` modes (the latter also proving the full X-set + real brackets survive); 8f corpus-differential still green; C5 calls `characterize_all_regions` (not static JSON); C3 asserts the gate is scored exactly once per gap on the default dump path |
...
6b.3. **Not yet started.** 8a–8f are landed; this is the plan of record for the flip.
...
| Sub-step | Scope | Clears | Validates with |
|----------|-------|--------|----------------|
| **8g.1** | **LANDED (2026-07-09).** Decode differential harness (shadow infra before any flip). Shared `harness::corpus_projection` (`project_corpus` / `golden_baseline_of_corpus` / `projection_diff`) — `gap_repair_spec_diff` refactored onto it. `fixtures::fingerprint_corpus_fixtures::synth_ab_corpus(diagnostics)` runs the **old** `characterize_gaps_with_gate` on an in-memory synthetic A/B → corpus. New **always-on** test `decode_path_projection` (default CI, no media): oracle → project → `projection_diff` empty. **Dev-dep-cycle learning:** an in-crate `#[cfg(test)]` sees a *different* `clip_sync_repair` instance than the harness links (types don't unify), so the driver moved to the **fixtures crate** + an **external** test, and `characterize_gaps_with_gate` was made **`pub`**. This is the harness 8g.3 extends (run the NEW from-decode path on the same synthetic input, diff vs this OLD oracle). | — | **Done:** `decode_path_projection` green in default CI (synthetic, no media); media differential still green; lib 355/355; clippy clean |
| **8g.2** | **LANDED (2026-07-09).** `characterize_all_regions(progress, media, regions, request, ctx) -> Vec<GapRepairSpec>` — the §2.5.5 loop over `characterize_region`, region-infallible (one spec per region; failures fold into `Skip`). Shadow: private `fn` + `#[allow(dead_code)]` (bumps to `pub(crate)`, with `RegionPatchContext`/`RegionPatchMedia`, when the dump wires it at 8g.3 — kept private now to avoid a `private_interfaces` leak). Unit test builds a synthetic A/B single region and asserts `specs.len() == regions.len()` and each spec's verdict agrees with the `prepare_region_patch` shim's execution (`Patch` ⟺ emits a `RegionPatch`; `Skip` ⟺ `Skipped`). | C5 dep | **Done:** `characterize_all_regions_yields_one_consistent_spec_per_region`; lib 356/356; clippy clean |
...
| **8g.3b** | **LANDED (2026-07-10). From-decode tags — equal by construction.** Refactored `tags_from_fingerprint` → shared `tags_from_fields(...)` core + thin wrapper; added `tags_from_measurements(&RegionMeasurements, levels)` as a second wrapper over the SAME core. Since both call `tags_from_fields` and differ only in `structure`/`seams` (both `None` under `skip_baseline_placement`), **from-decode tags == oracle tags by construction** — no separate from-decode orchestration needed to prove it here. The `tags_from_fingerprint` refactor is guarded by the 8g.1 / media / C4 differentials (all still green). A focused unit test pins the `tags_from_measurements` field mapping (brackets/seam_local/donor/registration/residual/levels; structure/seams omitted). **End-to-end from-decode differential deferred to 8g.4** (where the dump actually runs from-decode; the media/synth differential confirms empirically then). | (e) *from-decode half* | **Done:** `tags_from_measurements_maps_the_shared_fields`; tags_from_fingerprint refactor green across 8g.1 + media + C4 + lib 357/357; clippy clean |
| **8g.4** | **Flip the dump (lean).** `dump_gap_fingerprints` → decode → `characterize_all_regions` + `compute_region_measurements` → `tags_from_measurements` → `spec_to_fingerprint_summary`; **stop running the per-bracket gate oracle unconditionally** (the §1.3 dominant cost). Default = lean (no X, synthesized brackets). Old inline path deleted once green. | oracle-unconditional; **perf win** | Decode differential (lean) old-corpus vs new-dump on real media + re-anchor |
| **8g.5** | **Diagnostics fidelity.** On `--fingerprint-diagnostics`, `measure_region_tags` also computes the X-set (`seam_probe`/`wide_envelope`/`b_levels`/diagnostic `lag`) **+ the real `Vec<BracketInfo>`** (the per-bracket enumeration moves *here*, run on demand); `spec_to_fingerprint_summary` gains the `real_brackets` bypass. | fidelity constraint (troubleshooting survives) | Decode differential (`--fingerprint-diagnostics`) equal **incl. X + real brackets** |
| **8g.6** | **C5 + C3 + cleanup.** C5 (live characterize invariance — calls `characterize_all_regions`, not static JSON); C3 (gate scored **exactly once** per gap on the default dump); delete residual oracle code; final differential green in **both** modes. | §4.7 **C3**, **C5**, §3 step 7 | §4.7 C3/C5; both-mode decode differential + 8f corpus differential all green |
