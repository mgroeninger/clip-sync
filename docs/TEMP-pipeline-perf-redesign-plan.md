...
| Sub-step | Scope | Clears | Validates with |
|----------|-------|--------|----------------|
| **8g.1** | **Decode differential harness first** (shadow infra before any flip): decode a corpus's source (synthetic WAV fixtures for CI; real media via env for local) → run **old** `characterize_gaps_with_gate` → project → diff `golden_baseline`. Establishes the media-runnable gate the later sub-steps assert against. | — | Green on `tests/gap_corpus/wav` fixtures (CI) + a real licensed run (local) |
| **8g.2** | `characterize_all_regions(plan, decoded, request) -> Vec<GapRepairSpec>` — the §2.5.5 multi-region loop over `characterize_region`. Shadow only (not wired to the dump). | C5 dep | Unit: one spec per region; per-spec verdict matches the `prepare_region_patch` shim |
| **8g.3** | **From-decode overlay (the crux).** Extract `compute_region_measurements(decoded, spec, diag) -> RegionMeasurements` (raw primitive outputs) from the oracle loop; the **old** path builds `GapFingerprint` from it (stays authoritative), a new `tags_from_measurements` builds `GapRepairTags`. **Shadow-assert:** for every gap, from-decode tags reproduce `tags_from_fingerprint(old_oracle_fp)`'s `golden_baseline`. `structure`/`seams` omitted to match `skip_baseline_placement`. | (e) *from-decode half* | Decode differential (lean): from-decode projection == old oracle `golden_baseline` |
