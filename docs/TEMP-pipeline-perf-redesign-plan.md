...
| **8g** | Flip `dump_gap_fingerprints` → `characterize_gaps_with_gate` to call the shared `characterize_all_regions` (**new — only per-region `characterize_region` exists today**, `patch_audio.rs:2156`) + **compute the R/X tags from decode directly** (the shared primitives `lag_at_placement` / `donor_interior_at` / `splice_dualfit_at` / bracket oracle / `level_profile`, **order-independent** per §2.5.7 #5 H2 — absorbed from 8f) + `spec_to_fingerprint_summary`; **delete the second gate oracle** (`gap_fingerprint.rs:2132–2176`). **Validation reuses the 8f differential** (`gap_repair_spec_diff`) — it is source-agnostic, so old-oracle-corpus vs new-from-decode is the same gate; C5 makes it live. | §4.7 **C3**, **C5**, §3 step 7 | 8f differential still 69/69 after the flip; C5 calls `characterize_all_regions` (not static JSON); C3 asserts the gate is scored exactly once in the dump path |
