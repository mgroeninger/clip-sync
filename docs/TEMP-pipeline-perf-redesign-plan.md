...
NEXT: 8g.5 is **on hold — two approaches refuted by measurement (reclassification + FFT seam-probe pre-filter);
reassessing the lever or deferring** (see the 8g.5 row). Independently ready: 8g.6 (delete residual oracle +
`--validate-from-decode`, C5/C3-dump) and §3 step 8 (Hoists). Work through 8g.4a
was additive/byte-neutral; 8g.4b flips the default dump to the from-decode path (behavior-preserving, not yet
faster).**
...
| **8g.6** | **C5 + C3-dump + cleanup.** C5 (live characterize invariance) — **corrected (Finding 1): validates the live from-decode DUMP path (summary + `compute_region_measurements` + `tags_from_measurements`), not `characterize_all_regions`** (which the dump doesn't use). **C3-dump** (gate scored **exactly once** per gap on the lean dump — **not** §4.7 **C3**, which is the `fingerprint_diagnostics` X-set flag); delete residual oracle code; final differential green in **both** modes. Decide `characterize_all_regions`'s fate: keep as the scan-only repair-preview building block (§2.5.5) or delete if no consumer. | §4.7 **C3-dump**, **C5**, §3 step 7 | §4.7 C3-dump/C5; both-mode decode differential + 8f corpus differential all green |
