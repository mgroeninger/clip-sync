...
|-----|--------|------------------|
| [TEMP-fingerprint-field-clarity-plan.md](TEMP-fingerprint-field-clarity-plan.md) | 2026-08-08 | Fingerprint field clarity: rename five high-confusion wire keys (`equivalence`→`equivalence_diagnostic`, `scan_equivalence`→`equivalence_production`, `lag`→`lag_editorial`, `baseline_lag`→`lag_decision`, `donor_interior`→`donor_interior_aligned`) behind serde aliases (R1–R3), then co-locate a four-axis `_contract` (`measures`/`placement`/`window`/`not`) on eight metric groups via `Contracted<T>` + the `contracts!` registry (C1/C2). Kept for the rationale: why the wrapper was mandatory rather than a field on each group (shared *domain* types, one of which is on the production `--json` surface), why `source.not_measured` **paths are data** a serde alias cannot reach, why three C2 numbers are tunable config and so are labelled defaults that point at the recorded wire field instead of asserting a value, and the three substring traps — the new name containing the old (R1/R2) and the *un-renamed sibling* containing it (R3). C3 (share strings with `legend_text()`; `--fingerprint-contracts` knob) **deferred**: [../../../BACKLOG.md](../../../BACKLOG.md) § *Fingerprint contract follow-ups (C3)*. Current behaviour: [../gap-fingerprint.md](../gap-fingerprint.md) § *`_contract`* / § *Shape*. |
...
| [TEMP-residual-abstention-reporting-plan.md](TEMP-residual-abstention-reporting-plan.md) | 2026-08-05 | Residual-abstention reporting: `informative: false` split into four named reasons (`beyond_lag_reach` / `no_reference_window` / `probe_non_finite` / `floor_above_ok_db`), surfaced on `GapTags` and in dumps; the multichannel `floor_source` collapse fixed alongside; `placement_slide_frames` / `max_lag_frames` and `residual_floor_ok_db` carried so replay abstains where production did. Kept for the rationale: why the reason mirrors the *gate guard* rather than `!informative`, why `floor_above_ok_db` is a measurement and not a provenance finding, and §0.1's three killed probe changes. Current behaviour: [../../json-output.md](../../json-output.md) § *SeamResidualVerdict* / [../gap-fingerprint.md](../gap-fingerprint.md) § *Gate recipe*. |
| [TEMP-residual-measured-unify-plan.md](TEMP-residual-measured-unify-plan.md) | 2026-08-06 | Unify mono/multichannel residual “measured” semantics **toward MC**: shared `SideFloorState` / `combine_informative`; `ProbeNonFinite` ignored like unmeasured on both constructors; census + §5.1 before the gate move. Current behaviour: [../../json-output.md](../../json-output.md) § *SeamResidualVerdict* / [../gap-fingerprint.md](../gap-fingerprint.md) § *Gate recipe*. |
| [TEMP-donor-apply-edge-exclusion-plan.md](TEMP-donor-apply-edge-exclusion-plan.md) | 2026-08-05 | Head/tail exclusion for donor Apply: when Apply is on, A silent cores that touch the scanned A extent classify at the nominal map (Observe) while still recording registration. Geometry over gap index / `bins` floor. Current behaviour: [gap-scan.md](../../gap-scan.md) § Donor registration / [pipeline.md](../../pipeline.md) §3. |
...
| [TEMP-equivalence-instrument-convergence.md](TEMP-equivalence-instrument-convergence.md) | 2026-07-31 | I1–I3: scan vs fine equivalence **instruments** after F14/F15. Current table lives in [../gap-fingerprint.md](../gap-fingerprint.md) § *`equivalence_diagnostic` vs `equivalence_production`* (renamed from `equivalence` / `scan_equivalence` in R1). |
