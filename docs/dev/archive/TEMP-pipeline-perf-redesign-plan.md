...
**8g.4a media gate GREEN (2026-07-11): `from_decode_vs_oracle` passed on 7 licensed 5.1 pairs (58 gaps),
diagnostics-on; the lean run is a strict subset of the diagnostics `GoldenBaseline` comparison (it only nulls
`b_gap_floor_db`/`b_noise_floor_db`), so diagnostics-green ⇒ lean-green by construction — a separate lean run
is optional smoke, not a gate.** **REORDERED (2026-07-11):** a pre-flip full-JSON diff of the 7-pair data
found the from-decode **X-set** (`seam_probe`/`wide_envelope`/`b_levels`) already **bit-identical to the oracle
(0/58)**, but the per-bracket **rows** diverge on ~every row (611 rows) because `spec_to_fingerprint_summary`
**synthesizes** brackets from counts (no `real_brackets` bypass yet). So the fidelity half of old-8g.5 (the
`real_brackets` carry) is now **bundled into 8g.4b** so the flip is faithful on the *whole* per-gap JSON, not
just decision axes; the **perf win** (cutting the per-bracket oracle — which the from-decode path still runs
unconditionally, `gap_fingerprint.rs:2688`) is **decoupled into 8g.5**.
**8g.5 status (2026-07-11): TWO approaches refuted by measurement; reassessing.** The per-bracket oracle is
**82% of dump time**, and the cost-bearing gaps are **timing-offset** skips (same audio shifted ~150–197 ms,
lag-corr 0.996–0.999) scoring 11–22 brackets each, all failing at lag-0 `waveform_floor`. (1) The
reclassification/short-circuit + golden-refresh idea is **SHELVED** (only helped minority patch-tier gaps).
(2) The **FFT seam-probe pre-filter is REFUTED** — it never fires on timing-offset gaps (high lag-corr), the
nominal probe isn't a valid upper bound across brackets (g4), and fundamentally the gate seam is at a
structure-search-chosen placement a fixed probe can't predict (g003: nominal 0.998 vs gate seam 0.10). **No
code changed for either; the 8g.4b flip stands.** Open: reduce bracket count, cheaper per-bracket search,
timing-offset routing, or **DEFER 8g.5** (it's a diagnostic-dump speedup, not production). See the 8g.5 row.
**8g.4b LANDED (2026-07-11): the default `--gap-fingerprints` dump now runs `characterize_gaps_from_decode`
(+ real-bracket carry via a new `spec_to_fingerprint_summary` `real_brackets` param). Validated: lib 357/357;
the synth `from_decode_matches_oracle_full_fidelity_brackets_and_xset` test (real bracket rows + X-set equal,
both modes); the synth `golden_baseline` differential (both modes); the 8f re-anchor differential; the media
`from_decode_vs_oracle` (7 pairs / 58 gaps); harness 11/11; workspace builds; clippy clean (one pre-existing
harness nit). The oracle `characterize_gaps_with_gate` + `--validate-from-decode` are RETAINED as the
differentials' reference until 8g.6. Real-bracket carry is proven on synth + mechanism (source-agnostic); an
optional media re-dump can confirm on real 5.1 if desired.**
**8g.6 LANDED (2026-07-11): deleted the oracle (`characterize_gaps_with_gate`) + `--validate-from-decode` +
the old-vs-new differentials/fixture (migration scaffolding — not a permanent guard); `compute_region_measurements`
(shared measurement site) stays. Ongoing guard is C5 (`decode_path_projection` — live from-decode dump,
both modes) + the 8f re-anchor differential; C3-dump satisfied by construction (oracle gone ⇒ single scoring
site). Validated: lib 357/357, C5 green, 8f green, harness 11/11, clippy clean.**
NEXT: **Fingerprint-unification (8g) is COMPLETE.** 8g.5 (per-bracket-oracle perf) is **deferred** — two
approaches refuted by measurement (see the 8g.5 row + `docs/dev/gap-fingerprint.md` § Performance). Remaining plan
work: **§3 step 8 (Hoists)** — share the per-side mono downmix (measure first). Work through 8g.4a was
additive/byte-neutral; 8g.4b flipped the default dump; 8g.6 removed the dead oracle path.**
...
| **Lazy diagnostics** | X-set behind flag | **Partial** — X fields gated; per-bracket oracle + lag + `splice_dualfit` still always on in fingerprint scan. **8g target (lean-by-default, full-on-diagnostics):** the per-bracket oracle + full X-set move *into* the `--fingerprint-diagnostics` overlay — run on demand, not deleted. Default dump = D/R + decision axes (lean); diagnostics-on dump carries the full `FingerprintXSet` **+ the real `Vec<BracketInfo>`** through the projection so troubleshooting (`seam_probe`/`wide_envelope`/diagnostic `lag` = *why* a seam is dead; real per-bracket `failure_stage` = which bracket failed where) survives the flip. Projection synthesizes brackets today — 8g adds a carry-real-brackets path for diagnostics. |
| **Compute-once-share** | One decode · binned-RMS · **mono downmix** · lag curve | Decode shared inside patch ✓; scan still re-decodes A/B; `skip_baseline_placement` dedup ✓; hoist **open + rescoped** (step 1/8): share the per-side mono downmix (exactly slice-able), **not** one border template/RMS grid — §3.1 feasibility. |
...
| **7** | Golden harness: diff `GapRepairSpec` projections (Tier 1/2) on live rescans. **Not a separate build — C4/C5 are produced by Fingerprint-unification (8e/8g);** fold into step 8 or narrow to Tier-1 `cell()`/verdict identity. See the Fingerprint-unification ledger's "Sequencing" note. | §4.7 C1 workflow + `golden_baseline_corpus_invariance` |
| **8 (Fingerprint-unification)** | `characterize_gaps_with_gate` → shared characterize + X export only. **Clears (resolution index):** (d) pin `skip_cell_from_tags` `seam_local`; (e) populate `tags_ctx` D/R payload; (f) fix SilenceSplice inert geometry (#1); make skips real `Skip`-verdict specs. **Decomposed into 8a–8g — see the Fingerprint-unification sub-step ledger below.** | §4.7 C3-dump; no second gate oracle on lean dump |

**Then** resume step 1 hoists inside `characterize_region`'s shared context — **rescoped**: share the
per-side mono downmix (exactly slice-able), **not** one border template / RMS grid for all consumers (§3 step 8,
feasibility dig).
...
the D/R payload for the projection. (f) **Geometry half RESOLVED (Fingerprint-unification 8c, 2026-07-08).** The `Patch(SilenceSplice)` spec no
longer carries inert geometry: `skip_or_dual_fit` returns a `DualFitDecision` (strategy, no spec), and
`characterize_region`'s `finalize_dual_fit` attaches the real `b_extract` (via the dual-fit input's
`b_mapped_start`) + `gap_offset` — the "return the strategy, wrap in characterize" approach. A fingerprint
projection now reads real geometry. `gap_index` stays `0` uniformly (bracket path too; assigned at 8e/8f).
**Skips half RESOLVED (Fingerprint-unification 8d, 2026-07-08):** `RegionCharacterization::Skip` carries a
`Skip`-verdict `GapRepairSpec`; the loop/shim derives the `RegionPatchOutcome` via `skip_outcome_from_spec`
(byte-identical). Every region now yields a projectable spec. **Remaining for 8f:** decline-path `tags_ctx` is
still partial (no `seam_local` ⇒ class-3/4 declines carry a `Decorrelated` cell); the 8f overlay populates the
full D/R payload so decline cells are correct. Skip **specs are built but not yet retained** for projection —
the loop derives-and-drops; 8f/8g collect them.
...
| (d) pin `skip_cell_from_tags`'s `seam_local` assumption with a test **[DONE 8b]**; (e) populate the `tags_ctx` D/R payload **[DONE 8f — via `tags_from_fingerprint` + differential; from-decode wiring at 8g]**; (f) fix the SilenceSplice inert geometry (#1) **[DONE 8c]**; make skips real `Skip`-verdict specs **[DONE 8d]** | **Fingerprint-unification** (step-6 tail / §2.5.4 sub-step 8) |
...
##### Fingerprint-unification sub-step ledger (§2.5.4 sub-step 8 — proposed 2026-07-08)

Decomposes the "step 8 (Fingerprint-unification)" row into a shadow-first, test-gated sequence, in the 6b.3
style. **Not yet started** — this is the plan of record for it; adjust as sub-steps land.

**Prerequisite framing (must hold across the whole step).** Unification removes the **second *gate* oracle**
from the dump path (`gap_fingerprint.rs:2132–2176`, the per-bracket `oracle_*` re-run) — **not** the
registration / `donor_interior` / `splice_dualfit` measurements. Those are legitimately fingerprint-scope:
§1.2 labels `baseline_lag` / `donor_interior` **R** (needed only on the dual-fit/repair path); for a clean
**Bracket patch** they are **X**. So populating `tags_ctx` for *every* gap on the production patch path would
push X-set work onto production and break the "production carries only D/R" thesis (§1.0). The split this step
must preserve:

- **`characterize_region` populates `tags_ctx` only from what the decision already computes** — which, per the
  code audit for 8a (2026-07-08), is **narrower than first scoped**: only the **dual-fit path** yields a
  fully-decision-local, single-source, complete payload (`seam_local` + `donor_aligned`, copied from the one
  `DualFitResult`). The bracket path's `GateTags` **counts** (`brackets_total`/`brackets_passing`,
  `closest_failure_stage`, `best_bracket_seam`, `structure_min`) are **not** computed by the production gate —
  only the fingerprint per-bracket oracle (`gap_fingerprint.rs:2140–2177`) enumerates them. `levels` (A gap/noise
  floor) and `registration`/`donor_nominal` are likewise not on the production decision path. **All of those move
  to the 8f overlay**, not 8a.
- **The full registration/donor/`splice_dualfit`/gate-counts/levels-for-every-gap payload the golden wants is
  computed on the fingerprint *export* path** (8f), reusing the shared characterize primitives as an R/X overlay
  — never on the plain patch path.

**8c approach — DECIDED (2026-07-08): return the strategy, wrap in `characterize_region`.** `skip_or_dual_fit`
returns the `SilenceSplice` *strategy* (no geometry); `characterize_region` (which holds `b_extract`/
`gap_offset`/`gap_index`) wraps it into the full spec. Chosen over threading geometry into `skip_or_dual_fit`
because geometry ownership stays in one place and it matches the region-infallible `-> spec` shape (§2.5.7 #3).

| Sub-step | Scope | Clears | Validates with |
|----------|-------|--------|----------------|
| **8a** | **LANDED (2026-07-08).** Populate the **dual-fit** D/R payload on the `Patch(SilenceSplice)` spec (`skip_or_dual_fit`): `tags_ctx.seam_local` + `donor_aligned`, single-source from the one `DualFitResult`. Surfaced `post_seam_global_r` + `aligned_donor` onto `DualFitResult` (both already computed inside `try_dual_fit` — step-real denominator L150, aligned-donor gate L165 — just not returned) so the tags are copies, not re-measurements. `gate_pass = true` (only reached post-gate); uniqueness validators `None` (Tier-3). **Scope correction vs first draft:** `GateTags` counts/`structure_min`/`best_bracket_seam`/`closest_failure_stage`, `levels`, `registration`, `donor_nominal` are **not** decision-local in production → deferred to **8f** (see prereq). Bracket-path `tags_ctx` stays `default()` until 8f. | (e) *partial (dual-fit only)* | **Done:** lib 351/351 · `patch_audio_integration` 26/26 (byte-parity, incl. `patch_audio_dual_fit_rescues_inverted_post_border`) · single-source `debug_assert_eq!(seam_local.pre_seam_r, splice_pre)` at the build site |
| **8b** | **LANDED (2026-07-08).** Pin the `skip_cell_from_tags` `seam_local` dependency: 3 guard tests asserting that with `seam_local = None` a would-be class-3 (SilenceSplice) and class-4 (donor-dead ProgramQuiet) signature both **collapse to Decorrelated**, while the nominal-quiet term (read before `seam_local`) is unaffected. Locks the requirement that characterize must populate `seam_local` on **every** bracket-exhausted skip (decline path wired at 8d) or those two classes silently mislabel. | (d) *test only; enforcement lands 8d* | **Done:** `gap_repair_spec` domain tests 14/14 (adds `classifier_none_seam_local_collapses_class3/4_to_decorrelated`, `classifier_none_seam_local_still_honors_nominal_quiet`) |
| **8c** | **LANDED (2026-07-08).** Fixed SilenceSplice inert geometry (note f) via the decided approach: `skip_or_dual_fit` now returns a `DualFitDecision::Rescued(Box<DualFitRescue>)` (strategy + tags + base `refined`/`crossfade` from the dual-fit input) or `Skipped(outcome)` — **no spec built at that call site**. `characterize_region` wraps it via `finalize_dual_fit`, attaching real `b_extract` (from `b_extract_start/end_secs` + the dual-fit input's extract-relative `b_mapped_start`) and the real `gap_offset_secs`; the all-zero placeholder is gone. `gap_index` stays `0`, same as the bracket path (assigned at 8e/8f — not SilenceSplice-specific). | (f) *geometry half* | **Done:** lib 354/354 · `patch_audio_integration` 26/26 (byte-parity — executor's SilenceSplice arm doesn't read `b_extract`/`gap_offset`, so PCM is identical) · clippy clean |
| **8d** | **LANDED (2026-07-08).** `RegionCharacterization::Skip` now carries a `GapRepairSpec` (verdict `Skip{cell,reason}`), not a bare outcome. `skip_region_spec` builds it (reason→verdict, residual→`tags_ctx.gate.residual`, cell via `skip_cell_from_tags`); `skip_outcome_from_spec` reproduces the identical `Skipped{reason,residual}` at the loop/shim boundary. `DualFitDecision::Skipped{reason,residual}` → `finalize_dual_fit` wraps it into a geometry-bearing skip spec (consistent with 8c). All four skip sites (dual-fit decline, ZeroLength, BExtractFailed, AlignedSegmentOutOfRange) build specs. **Decline-path cells are partial** (no `seam_local` ⇒ class-3/4 report `Decorrelated`; the §8b guard's enforcement side) — corrected by the 8f overlay; outcome-invariant, so byte-parity holds. | (f) *skips-are-specs half* | **Done:** lib 354/354 · `patch_audio_integration` 26/26 (byte-parity, outcome fields identical) · clippy clean |
| **8e** | **LANDED (2026-07-09).** `spec_to_fingerprint_summary(spec, sample_rate, channels, x: Option<FingerprintXSet>) -> GapFingerprint` — **application** free fn in `application/gap_fingerprint.rs`, pure projection (measures nothing; A7 single-source). Maps registration→`baseline_lag`+`splice`, seam_local→`splice_dualfit`, donor blocks whole, gate summary→synthesized `brackets`/`structure`/`seams`/`residual`, levels→summary `LevelProfile`; `outcome.tier`=patch/skip. X-set (`FingerprintXSet`) attached only when supplied. C4 skeleton (`gap_repair_spec_projection.rs`) wired to a **real cross-crate round-trip** (project→serde JSON→`analyze_dirs`→`GapRow`): all 9 classes' cell identity + Tier-1 axes (`bracket_exhausted`/`donor_continuous`/`dualfit_pass`/`program_quiet_skip`) + flag-invariance. Tier-2 continuous no-recompute is unit-tested directly on the projected `GapFingerprint` in the repair crate. **Deferred to 8f:** `outcome.skip_reason` emits the real reason tag vs the frozen golden's `"gate skipped"` placeholder (golden-refresh reconciliation); `GapFingerprint.levels` is mandatory so a levels-`None` spec projects a placeholder (round-trips as `Some` — only affects early mechanical skips, not the 9 classes). | step-6a "Remaining" + **C4** | **Done:** repair lib 355/355 (adds projection unit test) · harness C4 2/2 (9 classes + flag-invariance) · clippy clean (touched files) |
| **8f** | **LANDED (2026-07-09).** Built the tags↔fingerprint bridge + **in-process differential harness** (the media-free validation agreed with the user). `tags_from_fingerprint(&GapFingerprint) -> GapRepairTags` extracts the full D/R payload an oracle fingerprint carries (registration from `baseline_lag`+`splice`, seam_local from `splice_dualfit`, per-side donor, gate counts/structure/seam/best/closest, residual, levels), mirroring the corpus reader's reads; `fingerprint_to_spec` wraps it. **Differential** (`tests/gap_repair_spec_diff.rs`, validation-tier): for every gap in the frozen corpus (old-oracle output), extract tags → re-project → assert `golden_baseline` unchanged — **passes on all 69 real gaps, zero media** (`b_levels` carried via the X-set). Proves `GapRepairTags` is a complete decision carrier + the projection is faithful on real data — the risk-retirement for the 8g flip. Also fixed two latent serde gaps so the repair schema re-reads its own corpus (`ResidualInfo` dB + `SeamScores.per_channel` null→NaN, matching the harness's `Option` tolerance). **Broaden with any licensed media** via `GAP_FP_DIRS`. **Re-scoped:** "compute tags from decode directly" (dropping the oracle) folds into **8g** — the same differential validates it (source-agnostic). | (e) *full* | **Done:** differential 69/69 (Tier-1 bit-exact + Tier-2 ε via `golden_baseline`) · repair lib 355/355 · harness C4 2/2 · clippy clean |
| **8g** | **Fingerprint dump flip — in progress (`8g.1`–`8g.3b` **LANDED**; authoritative detail = **sub-step ledger** `8g.4a`→`8g.4b`→`8g.5`→`8g.6` below).** Re-route the dump through the **fingerprint path**: **summary** (`characterize_gaps` → geometry + levels) + **`compute_region_measurements`** + verdict from **`m.outcome`/`any_ok`** + **`tags_from_measurements`** + **`spec_to_fingerprint_summary`** — **NOT `characterize_all_regions`** (Finding 1: production patch gate applies G4 residual veto at selection; fingerprint `any_ok` enumeration does not). **Lean-by-default:** default dump = D/R + decision axes; **`--fingerprint-diagnostics`** carries full X-set + real `Vec<BracketInfo>` — the real-bracket carry is bundled into **8g.4b** (REORDERED 2026-07-11) so the flip is faithful on the whole per-gap JSON. Stop the unconditional per-bracket oracle at **8g.5** (perf; the flip alone does not — the from-decode path still runs it). **Validation:** 8f corpus differential necessary but insufficient (no decode); **8g.4a** = **old oracle vs new from-decode on the same freshly decoded PCM** (lean + diagnostics-on) — **not** frozen committed JSON vs new path (frozen `re-anchor` may still carry pre–`skip_baseline` `structure`/`seams`; Tier-2 `throat_structure_min` drift on rescan is expected until F1/`fp.seams` fix + golden refresh — out of scope for the behavior-preserving flip). `outcome.skip_reason` may change at flip (8e deferred). | §4.7 **C3-dump**, **C5**, §3 step 7 | **`8g.4a`** decode differential green (synth + licensed media, both modes) → **`8g.4b`** flip → **`8g.5`**/`8g.6` |

**Sequencing — step 7 is produced here.** §3 step 7 (golden harness on live spec projections) is **not a
separate build**: its **C4** lands at 8e and **C5** at 8g. Fold §3-step-7 into this step, or narrow a
standalone step 7 to Tier-1 `cell()`/verdict identity (which needs neither `tags_ctx` nor the projection fn).
This resolves the ordering inversion — step 7 as written in the §3 table depends on step-8 outputs.

###### 8g sub-step ledger (the dump-path flip — proposed 2026-07-09)

8g is the largest remaining sub-step (it flips an outward-facing export), so it decomposes shadow-first like
6b.3. **`8g.1`–`8g.3b` LANDED**; **`8g.4a`–`8g.6` open.** 8a–8f are landed; this is the plan of record for the flip.

**Prereq framing.** Keep the old `characterize_gaps_with_gate` **authoritative** until each new piece is
shadow-proven equal, then flip and delete (the 6b pattern). Two facts learned validating 8f on real media
(2026-07-09) shape this: (1) the 8f `gap_repair_spec_diff` is **corpus→tags→project (no decode)** — it proves
the projection faithful but **cannot** see 8g's core risk (*from-decode tags == the oracle's*), which starts
from PCM; 8g needs a **decode differential**. (2) A plain `--gap-fingerprints` run is **lean** (no
`--fingerprint-diagnostics` ⇒ no `b_levels`/X-set; and `structure`/`seams` stay `None` under
`skip_baseline_placement`), so from-decode must **match those omissions**, and the fidelity path must be
validated by a **separate diagnostics-on run**.

**Frozen corpus vs decode differential (2026-07-10):** committed `re-anchor` `corpus.json` may still carry
`structure`/`seams` from pre–`skip_baseline_placement` scans; today's oracle omits them. **Do not validate 8g
by diffing new output against frozen JSON alone** — **`8g.4a`** runs **old oracle vs new from-decode on the same
freshly decoded PCM**. Tier-2 `throat_structure_min` drift on rescan is expected until the deferred F1
`throat_seam`→`fp.seams` fix + golden refresh (not bundled in the behavior-preserving flip).

**Open decisions — DECIDED (2026-07-09):**
- **Overlay home (8g.3) = extract shared measurements.** Pull `characterize_gaps_with_gate`'s per-gap
  measurement block into one shared producer `compute_region_measurements(decoded, spec, diagnostics) ->
  RegionMeasurements` (raw primitive outputs — `LagFingerprint` / `DonorInterior`×2 / `SpliceDualfit` /
  brackets / `LevelProfile` / X). The **old** dump builds `GapFingerprint` from it and stays byte-identical /
  authoritative; the **new** dump builds `GapRepairTags` → `spec_to_fingerprint_summary`. One measurement site
  (DRY, no divergence); cost is untangling the existing inline loop. *(Chosen over a parallel from-scratch
  overlay, which duplicates the oracle during transition.)*
- **Real-bracket carry (bundled into 8g.4b — REORDERED 2026-07-11) = dedicated `real_brackets` param.**
  `spec_to_fingerprint_summary(spec, sr, ch, x, real_brackets: Option<Vec<BracketInfo>>)`; `Some` ⇒ emit the
  real rows, `None` ⇒ `synth_brackets` fallback. Keeps bracket **counts** (a decision axis, D) first-class
  rather than conflating them into the X-set. *(Chosen over carrying brackets inside `FingerprintXSet`.)*
  Moved from 8g.5 to 8g.4b so the flip is faithful on the whole per-gap JSON (synthesized rows diverge 611/611
  on the 7-pair media data); 8g.5 is now the per-bracket-oracle **perf** gating only.

| Sub-step | Scope | Clears | Validates with |
|----------|-------|--------|----------------|
| **8g.1** | **LANDED (2026-07-09).** Decode differential harness (shadow infra before any flip). Shared `harness::corpus_projection` (`project_corpus` / `golden_baseline_of_corpus` / `projection_diff`) — `gap_repair_spec_diff` refactored onto it. `fixtures::fingerprint_corpus_fixtures::synth_ab_corpus(diagnostics)` runs the **old** `characterize_gaps_with_gate` on an in-memory synthetic A/B → corpus. New **always-on** test `decode_path_projection` (default CI, no media): oracle → `fingerprint_to_spec` → project → `projection_diff` empty (projection round-trip smoke only — **not** old-vs-new from-decode; that lands at **8g.4a**). **Dev-dep-cycle learning:** an in-crate `#[cfg(test)]` sees a *different* `clip_sync_repair` instance than the harness links (types don't unify), so the driver moved to the **fixtures crate** + an **external** test, and `characterize_gaps_with_gate` was made **`pub`**. | — | **Done:** `decode_path_projection` green in default CI (synthetic, no media); media differential still green; lib 355/355; clippy clean |
| **8g.2** | **LANDED (2026-07-09).** `characterize_all_regions(progress, media, regions, request, ctx) -> Vec<GapRepairSpec>` — the §2.5.5 loop over `characterize_region`, region-infallible (one spec per region; failures fold into `Skip`). Shadow: private `fn` + `#[allow(dead_code)]` (would bump to `pub(crate)`, with `RegionPatchContext`/`RegionPatchMedia`, **if** a preview consumer wires it — kept private now to avoid a `private_interfaces` leak; the dump does NOT use it, see re-scope below). Unit test builds a synthetic A/B single region and asserts `specs.len() == regions.len()` and each spec's verdict agrees with the `prepare_region_patch` shim's execution (`Patch` ⟺ emits a `RegionPatch`; `Skip` ⟺ `Skipped`). **ROLE RE-SCOPED (pre-flip review, 2026-07-10):** the pre-flip review (Finding 1) showed the DUMP must keep fingerprint `any_ok` semantics, so it does **not** use `characterize_all_regions` (which runs the production patch gate). This function's role is now the **scan-only repair-preview** building block (§2.5.5 "run characterize without execute") — specs without PCM. **No additional implementation needed** — the function is correct as-built; only its consumer changed. Stays `#[allow(dead_code)]` until a preview/C5-variant uses it, else a one-line delete at 8g.6 cleanup. | preview / (formerly C5 dep) | **Done:** `characterize_all_regions_yields_one_consistent_spec_per_region`; lib 356/356; clippy clean |
| **8g.3a** | **LANDED (2026-07-10). Byte-identical extraction proven.** Pulled `characterize_gaps_with_gate`'s per-gap overlay loop into `compute_region_measurements(RegionMeasureInput) -> RegionMeasurements` (raw primitive outputs); the loop assigns `fp` fields **from** the struct. Two faithfully-handled couplings: the throat-bracket seam (returned as `throat_seam`, applied onto `fp.seams` by the caller) and `fp.levels.gap_floor_db` (passed as input). **Byte-identity validated by a git A/B:** a temp dump of `synth_ab_corpus` (both lean + diag) compared old (HEAD, inline oracle) vs new (extracted) — **IDENTICAL in both modes**, `GapFingerprint` bytes unchanged. Temp harness removed. | — | **Done:** old-vs-new synth dump IDENTICAL (lean + diag); lib 356/356 (incl. diagnostics-toggle); `decode_path_projection` green; clippy clean |
| **8g.3b** | **LANDED (2026-07-10). From-decode tags — equal by construction.** Refactored `tags_from_fingerprint` → shared `tags_from_fields(...)` core + thin wrapper; added `tags_from_measurements(&RegionMeasurements, levels)` as a second wrapper over the SAME core. Since both call `tags_from_fields` and differ only in `structure`/`seams` (both `None` under `skip_baseline_placement`), **from-decode tags == oracle tags by construction** — no separate from-decode orchestration needed to prove it here. The `tags_from_fingerprint` refactor is guarded by the 8g.1 / media / C4 differentials (all still green). A focused unit test pins the `tags_from_measurements` field mapping (brackets/seam_local/donor/registration/residual/levels; structure/seams omitted). **End-to-end from-decode differential deferred to 8g.4a** (where the dump path is shadowed; the media/synth old-vs-new differential confirms empirically then). | (e) *from-decode half* | **Done:** `tags_from_measurements_maps_the_shared_fields`; tags_from_fingerprint refactor green across 8g.1 + media + C4 + lib 357/357; clippy clean |
| **8g.4a** | **Shadow the from-decode pipeline + the REAL old-vs-new decode differential (do this BEFORE flipping — reviewer feedback 2026-07-10).** Write `characterize_gaps_from_decode(...) -> GapCorpus` (pub, repair) = the corrected pipeline: per gap **summary** (`characterize_gaps` → geometry + levels, fingerprint refine) **+ `compute_region_measurements` → `m`** (8g.3a) **+ spec: verdict from `m.outcome.tier` (any_ok), tags from `tags_from_measurements(m, levels)`** (8g.3b) **+ `spec_to_fingerprint_summary`**. **NOT `characterize_all_regions`** — its `evaluate_seam_gate` applies the residual veto (G4) at selection, which `any_ok` does not, so a residual-vetoed gap is oracle `tier="patch"` but patch-gate `Skip` (Finding 1); levels/geometry from the summary dodge the refine/config divergence (Finding 3). Fixtures `synth_ab_from_decode_corpus(diag)` mirrors it; a test diffs `golden_baseline(from_decode)` vs `golden_baseline(oracle)` on the **same synth PCM**, **both lean and diagnostics-on** — the empirical proof that supersedes 8g.3b's by-construction argument (it also covers `tier`/`levels`/`geometry`, not just tags). **Throat decision (Finding F1, DECIDED): keep omitting `structure`/`seams` (`seam_min = None`) to match the current oracle** — the flip stays behavior-preserving. Populating `fp.seams` from `throat_seam` (the discarded gate decision seam) is a **separate, deferred** correctness fix (changes the corpus → golden refresh), NOT bundled here. **LANDED (2026-07-10):** `characterize_gaps_from_decode` (pub shadow) + `synth_ab_from_decode_corpus` fixture + `from_decode_pipeline_matches_oracle_golden_baseline` (default CI, both modes — the empirical proof). **Media gate wired (CLI, per reviewer feedback — no env var):** `--validate-from-decode` (CLI flag) dumps BOTH pipelines from ONE decode into `DIR/oracle` + `DIR/from_decode` (identical source-id ⇒ zero confound); `from_decode_vs_oracle` (validation-tier, `VALIDATE_FP_DIRS`) diffs them, **aligned + verified by source-id** (the decoded-audio hash already in each `corpus.json` — a mismatch hard-errors "different media"; folder names don't matter). Default dump path unchanged. Removed at 8g.4b. | — | **Done:** synth old-vs-new differential green (lean + diagnostics); lib 357/357; clippy clean; media differential (8f) still green |
| **8g.4b** | **LANDED (2026-07-11). Flip the dump + carry real brackets (fidelity bundled in — REORDERED 2026-07-11).** Media gate is **GREEN** (8g.4a, 7 pairs / 58 gaps). Two coupled changes, landed together so the flip is faithful on the *whole* per-gap JSON: **(1) real-bracket carry** — `spec_to_fingerprint_summary` gains a `real_brackets: Option<Vec<BracketInfo>>` param (the DECIDED dedicated-param design, §"Open decisions"); the one production caller `characterize_gaps_from_decode` passes `Some(m.brackets)` (already computed in `compute_region_measurements`), the other 4 call sites pass `None` (synth fallback stays for the corpus-projection path, which can't recover rows from stored tags). Without this, the flip would replace the oracle's real per-bracket rows (timings/`seam_*`/`failure_stage`) with synthesized-from-counts rows in **both** lean and diagnostics dumps — empirically 611/611 rows diverge on the 7-pair data (X-set itself is already 0/58, faithful). **(2) the flip** — `dump_gap_fingerprints`'s **default** path calls `characterize_gaps_from_decode` instead of `characterize_gaps_with_gate`. **The oracle function `characterize_gaps_with_gate` + the `--validate-from-decode` A/B harness are RETAINED** — the default-CI differentials (`decode_path_projection`, `from_decode_pipeline_matches_oracle_golden_baseline`) and the media `from_decode_vs_oracle` test still need the oracle as their reference. Deleting the residual oracle code (and `--validate-from-decode`) is **8g.6** cleanup, after C3-dump/C5 confirm the flipped path holds. **Perf note (corrected):** this flip is behavior-**preserving** but does **NOT** deliver the per-bracket-oracle speedup — `compute_region_measurements` still runs `oracle_score_fit_candidate` per bracket unconditionally (`gap_fingerprint.rs:2688`, needed for `any_ok`→tier + bracket counts). That speedup is 8g.5. | (fidelity: real brackets in the flipped dump); real-bracket carry (formerly 8g.5) | 8g.4a `GoldenBaseline` differential green on licensed media (**done** — diagnostics-on ⊇ lean) + re-anchor + a **full-fidelity diff** (X-set + real bracket rows equal, per-gap JSON, both modes — the 7-pair ad-hoc diff formalized as a test). **Done:** `from_decode_matches_oracle_full_fidelity_brackets_and_xset` (synth, both modes) + lib 357/357 + media `from_decode_vs_oracle` 7/7 + 8f re-anchor + harness 11/11; clippy clean. |
| **8g.5** | **⛔ VERIFICATION RESULT (2026-07-11): the FFT seam-probe pre-filter is REFUTED — do NOT build it. Reassess the lever (see below).** The make-or-break upper-bound check on the a licensed 5.1 pair corpus failed for two independent reasons: **(1) It never fires on the cost.** Every cost-bearing gap (g1–g7, all 97 brackets) has near-perfect correlation (0.996–0.999) at a **~150–197 ms offset** — they are **timing-offset** gaps (same audio, shifted), not decorrelated. A "skip if corr < floor" probe never triggers; the only low-corr gap (g8, 0.23) has **0 brackets** (zero cost). **(2) The bound doesn't hold.** g4 has a bracket at `seam_post=1.000` vs probe peak 0.996 — different brackets use different border templates, so a nominal probe is not a valid upper bound → the pre-filter would be **unsafe** (could flip `tier`/`bracket_exhausted`). **Root cause:** the gate seam is evaluated at a placement the **structure search chooses**, not the nominal/probe position — g003's nominal border correlates 0.998 while its gate seam is 0.10. You cannot predict the gate seam from a fixed-position probe because the search *moves* where the seam is measured. **Meta:** two successive 8g.5 approaches (reclassification, then this pre-filter) are now refuted by measurement; the dominant cost is **timing-offset gaps scoring many brackets**, which is hard to cut cheaply/safely. Open question for reassessment: (a) reduce bracket count per gap; (b) cheaper per-bracket search (shared structure timeline / bounded search); (c) recognize timing-offset upfront and route to dual-fit without the full grid (accuracy-adjacent); **or (d) DEFER 8g.5 entirely** — it is a *diagnostic-dump* speedup (not production), and the flip (8g.4b) already stands. **Broader-corpus check DONE (2026-07-11, existing 7-pair `full-compare` data, no new decode): 100% of the 470 skip-gap brackets are on timing-offset (corr ≥ 0.98) gaps — ZERO expensive truly-decorrelated (low-corr) skips exist anywhere in the corpus.** So the pre-filter is dead **corpus-wide**, not just on a licensed 5.1 pair; there is no salvageable slice. (Skips are 62% of all bracket cost; patch gaps 38%.) **RECOMMENDATION: DEFER 8g.5** — remaining levers (bracket-count reduction, timing-offset routing) are large/accuracy-adjacent for a diagnostic-only win; pick up 8g.6 + §3 Hoists instead. *(Original retarget rationale preserved below for context.)* <br><br>~~**Perf: conservative FFT seam-probe pre-filter for decorrelated skips (RETARGETED 2026-07-11 after measurement — the reclassification/short-circuit + golden-refresh approach is SHELVED, see below).**~~ <br>• **MEASUREMENT (a licensed 5.1 pair lean dump, `FP_TIMING`).** Per-bracket oracle = **82% of total dump time** (816 s of 993 s; decode only 12%), ~**8.4 s per `oracle_score_fit_candidate`**. All 9 gaps `tier=skip`; **100% of the 97 scored brackets failed at `waveform_floor`** — every bracket PASSED structure and failed the sample-level seam. This is the **decorrelated** regime (two recordings: envelope aligns, waveform doesn't), the core use case. So a *structure* pre-filter is useless; the discriminator is the **waveform seam**. <br>• **SHELVED — reclassification + golden-refresh (NOT doing it).** The short-circuit ("stop at first passing bracket") only helps **patch-tier** gaps; **exhausted skips must score every bracket**, and skips are the cost (100% here, 71% corpus-wide) — it targeted the wrong population. With the short-circuit gone, the reclassification loses its *only* motivation (it existed solely to let lean stop scoring early without breaking the golden); `brackets_passing` stays **exact for free**. **No golden refresh; `tier1!(brackets_passing)` stays.** *(Prior reclassification analysis preserved in git history if ever revived.)* <br>• **NEW APPROACH — conservative FFT seam-probe pre-filter.** A lag-domain seam correlation is an **upper bound** on the achievable seam Pearson over a window. Per gap, run an FFT seam-correlation probe over the search radius (reuse `lag_correlation_curve_auto`, `domain/seam_local.rs` — the ~50–150× lag sweep). If **both** the pre- and post-border peaks are below the waveform floor, no placement in the radius can pass → **skip the N-bracket unified search**, reporting `brackets_total` (cheap geometry) + `brackets_passing = 0` (**proven** by the probe). <br>• **WHY IT'S BEHAVIOR-PRESERVING (no golden refresh).** Every decision axis is byte-identical: `tier=skip` ✓, `brackets_total` exact (geometry) ✓, `brackets_passing = 0` (was 0, now proven) ✓, `bracket_exhausted = total>0 && passing==0` = true ✓. Only per-bracket `failure_stage` **rows** coarsen (diagnostic — the full grid still runs under `--fingerprint-diagnostics`). The existing `from_decode` differential is the safety net: any `brackets_passing`/`bracket_exhausted`/`tier` shift on a corpus gap ⇒ the bound was invalid. <br>• **MAKE-OR-BREAK (verify FIRST).** The upper-bound must hold across brackets that use **different border templates** (different `move_frames` → different A gap edges). Either prove the nominal lag-sweep peak bounds **every** bracket's gate seam on the corpus, or run the probe **per distinct border** (still ≪ the full unified search). This is the "correlation-based prefilter" §2.2a named as right-but-unbuilt — it needs a real safety proof, not a hand-wave. If the bound can't be made valid conservatively, fall back (no pre-filter; the flip already stands). <br>• **ORTHOGONAL — `TEMP-gap-equivalence-plan.md`.** That is a separate **plan-time** skip for gaps where **B already matches A** (high seams, redundant fills / scan false positives) — it helps *looser* scans (a licensed 5.1 pair min_gap 500 ms ≈ 25 redundant extras) but does **not** touch the decorrelated-skip cost (low seams → it correctly falls through to patch). Do **not** conflate; it does, however, build the same cheap-seam-probe primitive. | perf (decorrelated-skip bracket grid); §1.3 dominant cost | **(1) Verify** cross-bracket upper-bound on the corpus (nominal lag-sweep peak ≥ every bracket's gate seam, or per-border probe). **(2) Prototype** + measure prune rate / wall-clock on the skip-heavy a licensed 5.1 pair run. **(3) `from_decode` differential** (`brackets_passing`/`bracket_exhausted`/`tier` byte-identical) as the safety net; diagnostics-on still runs the full grid. |
| **8g.6** | **LANDED (2026-07-11). Delete the oracle + transitional scaffolding; C5 + C3-dump.** Per the migration-scaffolding principle (the old-vs-new differential was migration proof, not a permanent guard — once the new impl is trusted, keeping the old impl just to diff against it perpetuates the dual-path drift the unification removed), **deleted**: `characterize_gaps_with_gate` (the inline oracle `GapFingerprint` build; `compute_region_measurements` — the shared measurement site — stays), the `--validate-from-decode` CLI flag + composition branch, the `from_decode_vs_oracle` media differential, the two synth old-vs-new match tests, the `synth_ab_corpus` oracle fixture, and the now-dead `RegionMeasurements.throat_seam` (only the oracle read it; F1 will re-add if/when it populates `fp.seams`). **C5** = `from_decode_dump_projection_preserves_golden_baseline` (renamed `decode_path_projection`): runs the **live** from-decode dump (`characterize_gaps_from_decode` via `synth_ab_from_decode_corpus`) both modes, asserts well-formed + projection round-trip preserves every `golden_baseline` axis — the ongoing self-guard, no oracle reference. **C3-dump** = satisfied **by construction**: with the oracle deleted, the only per-gap scoring site is `compute_region_measurements`, called once per gap by the dump — there is no second gate oracle on the default `--gap-fingerprints` path. **`characterize_all_regions`** kept (`#[allow(dead_code)]`) as the scan-only repair-preview building block (§2.5.5). | §4.7 **C3-dump**, **C5**, §3 step 7 | **Done:** lib 357/357 · C5 `decode_path_projection` green (both modes) · 8f re-anchor differential green · harness 11/11 · clippy clean · workspace builds |

**After 8g:** Fingerprint-unification is complete; the remaining plan work is the **Hoists** step (§3 step 8 /
§3.1) — now unblocked, with the rescope (share the mono downmix, not the templates) and the priority caveat
(measure first; the per-bracket-oracle gating that **8g.5** delivers may already be the bigger win — note the
flip at 8g.4b is faithful but not yet faster, so the 8g.5 perf work is the real dominant-cost payoff).
...
| **C5** | Live characterize invariance — runs the live from-decode dump (summary + `compute_region_measurements` + `tags_from_measurements`), not static JSON. *(Corrected 2026-07-10: not `characterize_all_regions` — the dump keeps fingerprint `any_ok` semantics.)* | step 7 | Closes §4.6 “live fingerprint recompute” gap for decisions |
...
|------|------|--------|------------------|
| **1** | Hoist shared subexpressions (border extract, binned-RMS, dedup `place_on_b`) | **Partial** | `skip_baseline_placement` in `build_gap_fingerprint` / `characterize_gaps` ✓. Border extract + binned-RMS still rebuilt per consumer (§1.4). **Rescoped at §3.1 (2026-07-08):** the byte-preservable + worthwhile hoist is the shared **mono downmix**, not one border template/RMS grid for all consumers. |
...
The §1.4 framing ("Border templates are rebuilt in G1, `lag_at_placement`, `seam_probe`, `wide_envelope`,
`splice_dualfit` → **hoist one extract per gap**") over-promises. Traced against the code, "one max-radius
template, slice for all consumers" is **not byte-preservable**, for two independent couplings to the radius:

1. **Peak-relative trim threshold.** `trim_low_energy_suffix`/`_prefix` (`policies.rs:2157,2174`) set
   `threshold = peak * 0.12` over the *entire extracted window*, then trim trailing/leading samples below it. A
   wider radius can pull in a louder sample → higher `peak` → higher threshold → the trim point **near the gap**
   moves. So a small-radius template is not a suffix/prefix of the max-radius one.
2. **Silence walk bounded by `border_frames`.** `gap_border_frame_range` (`policies.rs:416–442`) walks `pre_end`
   back over silent frames but stops at `pre_start = gap_start − border_frames`. When the near-gap border is
   silent for a stretch comparable to the radius, a larger radius walks further into real content → different
   `pre_end`/`post_start`. Different result, not a slice.

Compounding it, the consumers pass genuinely **different `GapBorderSpec`** (different `border_frames` *and*
`standoff`/`floor`), so there is no single template that serves all: `border_frames` = `bin_frames*3`
(`gap_fingerprint.rs:996,1386`) vs `window + max_lag` (`:1210`) vs `border_secs·rate` (`:1722`) vs the
normalize-window (`patch_audio.rs:2375`). The plan is right to reject *that* hoist.

**What IS shareable (exactly, byte-identical): the mono downmix.** `interleaved_to_mono` (`policies.rs:244`) is
a pure per-frame mean, so `mono(samples)[a..b] == mono(samples[a..b])` — every consumer's sub-slice equals the
slice of one shared wide per-gap downmix. The expensive-input consumers already take a pre-downmixed buffer
(`donor_interior_at(b_mono, …)`, `donor.rs:40`), so the real redundancy is **B (and A) being downmixed
repeatedly per gap** (nominal donor, aligned donor, borders, levels each re-downmix their span). One shared
downmix in `characterize_region`'s context removes that with no threshold/phase hazard. The per-consumer
*shaping* on top — silence walk, peak-trim, and chunk-binning **anchored at each consumer's `start_frame`**
(`donor.rs:56` `span.chunks(bin)` with per-call phase; `DONOR_BIN_MS` ≠ `gap_signature_bin_ms=50` ≠ level bin)
— is **not** shareable, but is O(border)/O(span) cheap and does not need to be.

**Priority caveat.** Both the downmix and the binning sit in the plan's **cheap tier** (§1.3 #4: "O(N) RMS …
border"); the dominant per-gap costs are structure-search-per-bracket and the lag sweep (§1.3 #1–2). So even
the *safe* hoist targets a non-dominant cost. **Before spending effort here, measure whether the shared downmix
moves wall-clock at all**; if not, demote Hoists below the still-unclaimed **per-bracket-oracle** gating (§2.2
"Remaining perf targets" — the `N_brackets × O(N·radius)` re-run), which attacks §1.3 #1 directly.
...
not a single cell enum. Axis derivation background: [archive/TEMP-gap-vocabulary-redesign-plan.md](TEMP-gap-vocabulary-redesign-plan.md) §2;
...
**§4.7 ID disambiguation:** **C3** = `fingerprint_diagnostics` flag smoke (X-set gated; landed in lib —
`characterize_gaps_with_gate_include_diagnostics_toggles_x_set`). **C3-dump** = gate scored exactly once per
gap on the **lean** dump path (**8g.6** — distinct from C3; do not conflate).
...
| **C3** | `fingerprint_diagnostics` flag smoke | step 8 regression | **DONE (2026-07-10)** | `gap_fingerprint.rs` `characterize_gaps_with_gate_include_diagnostics_toggles_x_set` — flag off ⇒ X fields absent; flag on ⇒ `seam_probe`, `wide_envelope`, diagnostic `lag`, `b_levels` present. *(Distinct from **C3-dump** — gate-once on lean dump, **8g.6**.)* |
| **C3-dump** | Gate scored exactly once per gap on lean dump | 8g.6 | **OPEN** | After **8g.4b** flip — no unconditional per-bracket oracle on default `--gap-fingerprints` path. *(Not §4.7 **C3** — diagnostics X-set flag.)* |
...
| **C5** | Live characterize invariance (live from-decode dump vs rescan; *corrected 2026-07-10 — not `characterize_all_regions`*) | step 7 | **OPEN** | Extends C1 — closes §4.6 static-JSON-only gap for **decisions** |
...
Step 6 (§2.5):           6a → 6b → 6c → 7 (C5) → 8 (fingerprint unify + C3-dump)
