...
> **Status:** **Shipped** (2026-06-22). Phases 1–4 complete. Phase 5 deferred (verbose hint only; no auto `anchored_retry`). Archived copy: [archive/repair-profiles-plan.md](repair-profiles-plan.md).
...
| Anchored retry | `domain/fill_offset.rs`, `patch_audio.rs` | Two-pass; anchors from **High** only; see [archive/patch-anchor-offset-plan.md](patch-anchor-offset-plan.md) |
...
- [x] Add `FitBoundarySearch` enum: `BaselineOnly` | `FullGrid` (domain or config).
- [x] Thread through `SeamGateParams` / `PatchAudioRequest`.
- [x] In `evaluate_seam_gate_fit_joint` (`patch_region.rs`):
...
- [x] Default `RepairConfig`: `fit_boundary_search = BaselineOnly`.
- [x] Unit tests in `patch_region.rs` or `patch_audio.rs`:
  - Marginal baseline ⇒ no grid invocation (mock or cell-count assertion).
  - `FullGrid` + marginal baseline ⇒ grid still runs.
- [x] Integration: existing fit tests still pass; add one test with `FullGrid` mirroring pre-change behavior if needed.
...
- [x] `RepairProfile` enum + `apply_repair_profile(config: &mut RepairConfig, profile)`.
- [x] `[repair] profile = "default"|"quick"|"full"` in config (default `"default"`).
- [x] CLI: `--quick` → `Quick`; `--full` → `Full`; `--profile <name>` (optional explicit).
- [x] `apply_cli_overrides` order:
  1. Load TOML
...
- [x] Reject `--quick` && `--full`.
- [x] Tests: `apply_cli_overrides` profile + override precedence; conflict error.

### Phase 3 — Verbose diagnostics

**Intent:** User can see why a run is fast/slow without reading code.

- [x] `format_repair_profile_verbose(config, profile, overrides)` in `cli/output.rs` or `patch_audio.rs`.
- [x] Emit once at start of patch phase (after fill plan).
- [x] Per-gap: `fit path: baseline only` vs `boundary grid (N cells, haystack Xs)` in `evaluate_seam_gate_fit_joint` via `ProgressReporter::phase_verbose` callback or progress param.
- [x] Update [cli-output.md](../../cli-output.md) verbose section.

### Phase 4 — Docs + README

- [x] [gap-fill-modes.md](../../gap-fill-modes.md): replace manual “interactive / faster fit” recipe with profile table; keep flag overrides documented.
- [x] [README.md](../README.md): `clip-sync-repair ... --mux out.mp4` (default), `--quick`, `--full` examples.
- [x] Note: `anchored_retry` not in profiles; use `--fill-offset anchored-retry` override on `full` runs when needed.
...
- [archive/patch-anchor-offset-plan.md](patch-anchor-offset-plan.md) — `anchored_retry`
