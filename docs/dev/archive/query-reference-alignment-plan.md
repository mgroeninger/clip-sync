...
> **Status:** Shipped (2026-06-15) — Q0–Q4 complete. Archived from `docs/dev/archive/TEMP-query-reference-alignment-plan.md`.
>
> **Post-ship review (2026-06-16):** Found and fixed a false-ambiguity bug — partial-overlap neighbor search windows of a single true match (a confidence "shoulder" within one query length) were treated as a competing location, flagging clean matches `ambiguous` and halving confidence. This sank the 45-min corpus oracle (confidence 0.4999 < 0.5). Fixes: (1) `select_best_cluster` now requires a competing cluster to be ≥ one query length away (within-footprint shoulders ignored), guarded by direct unit tests; (2) corpus case `clip_length_secs` 60→600 so the 480 s query is not truncated to a tiny, chroma-indistinct coarse window; (3) the long query fixture uses a bounded (non-aliasing) chirp. Note: chirp fixtures are chroma-periodic (chromaprint is chroma-based), so they are a weak proxy for "clean match" — ambiguity logic is now unit-tested directly on synthetic candidates instead.
>
> **Still-open deferrals (not blocking the repair use case):** Region-bounded hold-out inside `mapped_region` is **shipped** (`mapped_region_holdout_candidates` / `resolve_holdout_candidates` in `policies.rs`; wired in high-rate + offset verify). Remaining: sub-sample parabolic peak interpolation (high-rate), multi-anchor donors.
>
> **Shipped follow-up (2026-06-16):** [query-reference-b-longer-plan.md](query-reference-b-longer-plan.md) — query mode runs when **either** file is shorter; B-longer repair (short A, long donor B) with explicit offset sign and A/B span remapping. Synthetic `ClipMatch` / `discovery_windows` rebased to A timeline via `winning_window_on_a_timeline`.
...
|------|-------------------------|
| [archive/media-session-redesign-plan.md](media-session-redesign-plan.md) | Shipped 2026-06-11: `MediaSession` `&mut self`; `reset_io` removed; `MediaExtent`; scan policy in `application/media_scan.rs` |
| [archive/verification-hardening-plan.md](verification-hardening-plan.md) | **Shipped (2026-06-11):** verify retry + `candidates_tried`, `clip_with_label`, `alignment_fixtures`, test-role split — see [corpus-validation.md](../corpus-validation.md) § Validation diagnostics |

**Parallel (not blocking query mode):**

| Plan | Relationship |
|------|----------------|
| [archive/periodic-ambiguity-plan.md](periodic-ambiguity-plan.md) | Symmetric pair mode: mod-**T** ambiguity on repeating **A**; query mode: `ambiguous` on repeating anchors in long **A** — align warning semantics in docs |

**Already shipped (rebase on these, do not re-litigate):**

- [archive/output-error-contract-plan.md](output-error-contract-plan.md) — JSON contract v1 frozen; new fields require explicit revision (see [JSON contract revision](#json-contract-revision)).
- [archive/layer-purity-plan.md](layer-purity-plan.md) — domain types carry **no serde**; JSON DTOs live in `application/report.rs`.
...
- [x] **docs/dev/corpus-validation.md** — describe `wav_query_reference_*` (generated tier) and test roles
- [x] **docs/dev/development.md** — brief pointer if corpus env vars apply
- [x] **BACKLOG.md** — link this plan until archived
- [x] Archive this doc → `docs/dev/archive/query-reference-alignment-plan.md`
...
- Prerequisites: [archive/media-session-redesign-plan.md](media-session-redesign-plan.md) (shipped), [archive/verification-hardening-plan.md](verification-hardening-plan.md) (shipped)
- Layer purity (shipped): [archive/layer-purity-plan.md](layer-purity-plan.md)
...
- Hold-out verify (shipped): [archive/offset-verification-plan.md](offset-verification-plan.md)
