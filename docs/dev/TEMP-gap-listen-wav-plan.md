...
| **A gate-refused gap gets both surround clips and no patched clip** (added at implement time, §12.2) | Donor at a different frequency + `disable_structure_trust`, asserting `Skipped` plus `_a_surround` / `_b_surround` present and `_a_patched` absent. **`#[ignore]`d** — see §12.2 | Synthesizing a patched clip from the oracle when production produced no patch; dropping the donor clip a refusal can only be judged against | `a_gate_refused_gap_gets_both_surround_clips_and_no_patched_clip` (ignored; runs in the **diagnostic** tier, release `--ignored` lane) |
...
**Where they run.** Both test binaries are in the `pr-repair` tier's `calibration` invocation
(`scripts\test-tier.ps1`), alongside `cli_gap_fingerprint_provenance` — they need their own
invocation because the files are `#![cfg(feature = "calibration")]` and would otherwise build empty
and pass vacuously. The batch is ~130 s in debug. The one `#[ignore]`d row runs in the **diagnostic**
tier's release `--ignored` lane, next to `w5_anchor_rescue_pipeline`. Declaring a `[[test]]` is not
enough to make it run: every tier names its targets explicitly, so
`scripts\check-repair-test-manifest.ps1` now also fails on a declared binary no tier lists.
