...
- **Open review ledger:** [TEMP-equivalence-divergence-findings.md](TEMP-equivalence-divergence-findings.md) — two open findings split out of the silence-floor ledger on 2026-07-30; both diagnosed in source 2026-07-30, neither fixed. **F15** (high): shared classifier, incompatible sensors (scan silent-core / abs-floor donor vs fine refined-span / gap-floor RMS) — `skip_equivalent_gaps` keeps a gap fill-time drops. **F14** (medium-high): fingerprint `outcome` is bracket `any_ok` only; dual-fit rescue is published (`gate_pass`) but never wired into the decision axis, so the corpus records `skip` where production patches.
