...
- **Open review ledger:** [TEMP-equivalence-divergence-findings.md](TEMP-equivalence-divergence-findings.md) — **F15** (high): floor answered **(a)**; keep/drop still open on A-RMS + context/noise-floor window. **F14** (medium-high): border alignment **fixed and media-validated** (`splice_dualfit_at` = production raw mono; flag now matches production, `trim_frames` frame-exact). Residual: shoulder-crossing + NaN edges unmodelled.
