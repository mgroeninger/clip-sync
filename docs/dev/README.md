...
- **Open review ledger:** [TEMP-equivalence-instrument-convergence.md](TEMP-equivalence-instrument-convergence.md) — the three items left open when the F14/F15 ledger closed, all one axis: the two equivalence front-ends now share corrected sensor *definitions* but sample them with different **instruments**. **I1** bin size — **done + media-validated 2026-07-30**: the equivalence overlay now bins at `scan_block_ms` (`gap_signature_bin_ms` untouched, it has production consumers). Result on the F15 pair: **3 class divergences → 1**, and `gap_floor_db` / `a_gap_rms_db` now agree **exactly** (0.00 on all ten gaps). **I2** noise-floor context window (2.0 s vs 3.0 s) — **decided: accept-and-document**; once bin size was separated out the residual re-measured at **0.606 dB** median (not 2.13, which was mostly bin), flipping one gap in the safe direction. **I3** the donor predicate's missing `b.silent ||` disjunct — **the only unmeasured item left**, and the one bias pointing the *opposite* way; measure before fixing, using the existing dump. Note `measure_gap_equivalence` has exactly one caller (the fingerprint dump), so none of this gates repair — the risk is lost **sensitivity** in `equivalence-calibration`'s CI gate, which fires only on the dangerous direction.
