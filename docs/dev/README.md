...
- **Active drafts:** `TEMP-*.md` in this folder. Currently: [TEMP-scan-recipe-plan.md](TEMP-scan-recipe-plan.md) (**unparked 2026-07-30**, ready to implement), [TEMP-equivalence-divergence-findings.md](TEMP-equivalence-divergence-findings.md), [TEMP-gap-selection-deferred.md](TEMP-gap-selection-deferred.md), [TEMP-nway-donor-alignment-plan.md](TEMP-nway-donor-alignment-plan.md).
...
- **Open review ledger:** [TEMP-equivalence-divergence-findings.md](TEMP-equivalence-divergence-findings.md) — **F15** (high → **medium**, population measured 2026-07-30: **5/297 divergent, 0 dangerous**, recipe-invariant across two corpora over the *same* 17 pairs — every action divergence is the conservative direction; rule of three bounds the dangerous rate < ~1 %, not zero): floor answered **(a)**; donor mechanism measured (donor sits between the two floors on exactly the divergent gaps); noise-floor axis characterized (fine lower on 10/10 gaps, ~8 dB, biasing toward `drop`) but still open, as is A-RMS. **Fork closed on (B)** — sensors stay different, interpretation fixed: `scan_equivalence` documented as authoritative, the fine block as diagnostic (both its biases point at `drop`), "the fine reference" retired everywhere, and the cell-fixture test corrected to assert scan-time cells against the scan verdict. Class pinned media-free by `tests/gap_corpus/fingerprints/equivalence_divergence/` + `tests/equivalence_divergence.rs`. **F14** (medium-high): border alignment **fixed and media-validated** (`splice_dualfit_at` = production raw mono; flag now matches production, `trim_frames` frame-exact). Residual: shoulder-crossing + NaN edges unmodelled.
