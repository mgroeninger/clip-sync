...
- **Open review ledger:** [TEMP-equivalence-divergence-findings.md](TEMP-equivalence-divergence-findings.md) — two open findings split out of the silence-floor ledger on 2026-07-30, both measured on media and neither yet traced to source. **F15** (high): scan-time and fill-time equivalence return opposite verdicts on one gap (`donor_silence_fraction` 0.10 vs 0.87), and `skip_equivalent_gaps` consumes the wrong one. **F14** (medium-high): the fingerprint corpus records `skip` where production patches via dual-fit, biasing the calibration oracle.
