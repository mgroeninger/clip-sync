...
| C3 | Does the **dual-fit repair pass the unchanged gate** on the known skips? | **PROVEN — yes, for a real subset** | CRIT | **Scan-native `splice_dualfit` (full corpus, 2026-07-01, on the scan's own decode): 7/39 bracket-exhausted skips pass the gate** at per-shoulder placement — **6 need the step (genuine silence-splices), 1 is a constant offset** (1·g21). Passes: 1·g3, **1·g19**, 1·g21, 2·g2, 6·g3, 6·g8, 7·g2. **Refutes the sim's decode-tainted "steps spurious" claim:** 1·g19 (step **+315.8 ms**) reads **both seams 0.998** on the scan decode (the sim had called it dead/common-offset). Genuine large-step splices exist and dual-fit clears the unchanged gate on them. Failures split into *one-shoulder-dead-at-seam* (1 s lag aligns, 250 ms seam dead one side — splice at the edge) and *donor BROKEN*; the latter will further split into program-quiet non-dropouts (D11) once the nominal-donor re-scan lands. Sim retired (E-tombstone). |
...
| C-harness-2 | **`verdict` / `skew` use one side** — `gap_row` takes `pre.or(post)` for the headline lag verdict and constant-vs-drift skew | OPEN | Low — prefer `splice` / `seam_step_ms` / `splice_diag` for classification; CSV `verdict` column can mislead |
| C-harness-3 | **Legacy `gap.lag` fallback** — analyzer uses `baseline_lag.or(gap.lag)` when projecting rows | OPEN | Low — do not mix pre-A2 JSON with post-A2 corpora in one `diag_fingerprint_corpus` run |
...
| **Edge-pinned peaks** | OPEN (A5/C6) | Flag `\|frac_lag_ms\| ≈ lag_max_lag_ms`; `splice.step_ms` unreliable when post peak missed. Coarse post placement only if rescan shows systematic edge-pins (C4: not needed for recoverable population). |
...
| **`dualfit_candidate` narrow** | by design | Requires exact `SpliceDiag::Splice` + bracket-exhausted; use **`splice_dualfit.gate_pass`** (dual-fit viability section) for the C3 answer. |
...
**Dual-fit addressable set (measured, not estimated):** scan-native `splice_dualfit` (2026-07-01) —
**7/39 bracket-exhausted skips actually pass the unchanged gate** (1·g3/g19/g21, 2·g2, 6·g3/g8, 7·g2), 6 of
them genuine real-step splices (C3). This supersedes the earlier "13 both-sides-recoverable" *estimate*: of
the recoverable-by-uniqueness set, the ones whose 250 ms seams also clean up at placement are these 7; the
rest fail with one shoulder dead at the seam or a broken donor. One-sided-dead collapsed to 8/55 (B2/C1) —
not a separate rescue path.

**One-line status:** registration **closed and validated** (A2/B2/B13/C1/C2/C4 PROVEN); **dual-fit
viability PROVEN in-scan (C3): 7/39 skips pass the gate, 6 genuine splices** — incl. 1·g19 (step 315 ms,
both seams 0.998), refuting the retired sim's spurious-step claim. **Next:** re-scan to populate
`donor_interior_nominal`/`b_levels` (D11 — reclassify donor-BROKEN fails as program-quiet non-dropouts) →
**wire §4 repair (A3)** scoped to the passing cohort → calibrate thresholds (A5/C6).
