...
`seam_pearson` correlates two equal-length windows after **peak-normalizing each side** (so encode-to-encode *level* differences don't matter; *shape* does). It returns **0.0** when the windows are empty or unequal length. Because it normalizes level, **near-silent audio correlates to ~0** — its waveform is dominated by noise, which differs between two sources.
