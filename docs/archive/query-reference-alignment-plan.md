...
>
> **Still-open deferrals (not blocking the repair use case):** **(a)** A-as-query orientation — query mode requires A = the longer file; when query mode is selected but A is shorter, `execute()` falls back to symmetric with a logged note. **(b)** Region-bounded hold-out placement for high-rate/verify — in query mode `apply_high_rate_refinement` always no-ops gracefully (the offset pushes all hold-out candidates to negative B time), so the `refine_offset_high_rate = true` repair default does nothing on the query path; PCM refine already delivers ±0.05 s, so precision is unaffected, but the report shows it skipped.
