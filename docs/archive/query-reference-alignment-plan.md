...
>
> **Still-open deferrals (not blocking the repair use case):** **(b)** Region-bounded hold-out placement for high-rate/verify — in query mode `apply_high_rate_refinement` always no-ops gracefully when hold-out candidates fall outside feasible A/B overlap (common with large negative offsets when A is reference; B-longer query mode has a similar limitation when discovery windows sit on B). PCM refine already delivers ±0.05 s, so precision is unaffected, but the report shows refinement skipped.
>
> **Shipped follow-up (2026-06-16):** [TEMP-query-reference-b-longer-plan.md](../TEMP-query-reference-b-longer-plan.md) — query mode runs when **either** file is shorter; B-longer repair (short A, long donor B) with explicit offset sign and A/B span remapping. See `align_videos.rs` + `from_reference_outcome`.
...
- [x] **A-as-query orientation** — shipped 2026-06-16 ([TEMP-query-reference-b-longer-plan.md](../TEMP-query-reference-b-longer-plan.md) B0–B4): `execute()` runs query mode whenever resolved mode is `QueryReference`; `align_query_reference` picks reference/query by length and frames via `from_reference_outcome` (offset sign + A/B remap). Repair unchanged.
