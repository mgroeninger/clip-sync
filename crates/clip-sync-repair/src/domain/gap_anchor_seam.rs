...
///
/// **Superset proof.** [`matchability_at_anchor`] rescues a below-`min_pearson` seam via GCC-PHAT xcorr only
/// inside the narrow band `[min_pearson − xcorr_ambiguous_band, min_pearson)`; strictly below
/// `min_pearson − xcorr_ambiguous_band` there is no rescue and Pearson alone fails. So
/// `matchable-at-placement p ⟹ side_pearson(p) ≥ floor` where `floor = min_pearson − xcorr_ambiguous_band`.
/// The searched placement's `start` lies in `[start_lo, start_hi]` (the exact reachable range — coarse+refine
/// search radius plus fine polish; `snap_fill_to_gap` never moves `start_frame`), so
/// `windowed_max(side_pearson) < floor ⟹ side_pearson(searched) < floor ⟹ not matchable`. A bracket needs
/// BOTH sides matchable, so either side's windowed-max falling below the floor dooms it. The skip set is thus
/// a subset of the real searched-placement reject set → the candidate pool is identical → output byte-identical.
///
/// **Conservative on ambiguity.** Returns `false` (do not skip; let the search run and decide) when the FFT
/// band cannot be computed uniformly across the window, and only skips when strictly below the floor by
/// [`MATCHABILITY_PREGATE_EPSILON`] — a margin comfortably above the band's ≤1e-8 FFT error, so numerical
/// slop can never turn a would-be-matchable bracket into a false skip.
///
/// `start_lo`/`start_hi` are inclusive frame bounds; the caller derives them from the search geometry and
/// clamps `start_hi` to `templates.b_mono.len()`.
pub(crate) fn anchor_bracket_matchability_doomed(
    templates: &crate::domain::policies::SeamTemplates<'_>,
    gap_frames: usize,
    pre_window: usize,
    post_window: usize,
    ...
    let floor = f64::from(params.min_pearson) - f64::from(params.xcorr_ambiguous_band);
    let score_channels = crate::domain::policies::seam_score_channels(templates);
    let Some(band) = crate::domain::policies::fill_seam_correlations_band(
        templates,
        gap_frames,
        pre_window,
        post_window,
        ...
        // Non-uniform band edges or a band that does not fit: cannot cheaply prove doom → run the search.
        return false;
    };
    let mut max_pre = f64::NEG_INFINITY;
    let mut max_post = f64::NEG_INFINITY;
    for (pre, post) in band {
        if pre > max_pre {
            max_pre = pre;
        }
        if post > max_post {
    ...
    let cutoff = floor - MATCHABILITY_PREGATE_EPSILON;
    max_pre < cutoff || max_post < cutoff
}

/// Skip margin below the matchability floor for [`anchor_bracket_matchability_doomed`]. Well above the FFT
/// band's ≤1e-8 reproduction error so band ε can never cause a false skip.
pub(crate) const MATCHABILITY_PREGATE_EPSILON: f64 = 1e-6;
...
mod tests {
    ...
    fn det(seed: u64, n: usize) -> Vec<f64> {
        let mut s = seed | 1;
        (0..n)
            .map(|_| {
    ...
    fn mono_templates<'a>(
        ...
        b_mono: &'a [f64],
        empty: &'a [Vec<f64>],
    ) -> crate::domain::policies::SeamTemplates<'a> {
        crate::domain::policies::SeamTemplates {
            a_pre,
            a_post,
            a_pre_ch: empty,
            a_post_ch: empty,
    ...
    /// A bracket whose seam templates correlate with B nowhere in the reachable window is doomed → skip.
    #[test]
    fn matchability_pregate_skips_fully_decorrelated_bracket() {
        let params = AnchorMatchabilityParams {
            min_pearson: 0.6,
            xcorr_ambiguous_band: 0.1, // floor = 0.5
            ..AnchorMatchabilityParams::default()
        };
        let (pre_window, post_window, gap_frames) = (256usize, 256usize, 40usize);
        let a_pre = det(1, 256);
        let a_post = det(2, 256);
        let b_mono = det(999, 4000); // independent noise: no placement correlates
        let empty: Vec<Vec<f64>> = Vec::new();
        let templates = mono_templates(&a_pre, &a_post, &b_mono, &empty);
        assert!(anchor_bracket_matchability_doomed(
            &templates, gap_frames, pre_window, post_window, 300, 3000, &params,
        ));
    }

    /// If ANY reachable placement is matchable on both sides, the bracket must NOT be skipped (superset
    /// safety) — even a single matchable start in a sea of decorrelated ones keeps the search alive.
    #[test]
    fn matchability_pregate_keeps_bracket_with_a_matchable_placement() {
        let params = AnchorMatchabilityParams {
            min_pearson: 0.6,
            xcorr_ambiguous_band: 0.1,
            ..AnchorMatchabilityParams::default()
        };
        let (pre_window, post_window, gap_frames) = (256usize, 256usize, 40usize);
        let a_pre = det(1, 256);
        let a_post = det(2, 256);
        let mut b_mono = det(999, 4000);
        // Make start S=1500 perfectly matchable on both sides by splicing the A templates into B.
        let s = 1500usize;
        b_mono[s - pre_window..s].copy_from_slice(&a_pre); // pre: b[S-w..S] == a_pre
        b_mono[s + gap_frames..s + gap_frames + post_window].copy_from_slice(&a_post); // post
        let empty: Vec<Vec<f64>> = Vec::new();
        let templates = mono_templates(&a_pre, &a_post, &b_mono, &empty);
        assert!(!anchor_bracket_matchability_doomed(
            &templates, gap_frames, pre_window, post_window, 300, 3000, &params,
    ...
    /// A non-uniform band edge (window narrower than `pre_window` at `start_lo`) makes the FFT band
    /// inapplicable; the pre-gate must fall back to NOT skipping (run the search).
    #[test]
    fn matchability_pregate_conservative_when_band_inapplicable() {
        let params = AnchorMatchabilityParams::default();
        let (pre_window, post_window, gap_frames) = (256usize, 256usize, 40usize);
        let a_pre = det(1, 256);
        let a_post = det(2, 256);
        let b_mono = det(999, 4000);
        let empty: Vec<Vec<f64>> = Vec::new();
        let templates = mono_templates(&a_pre, &a_post, &b_mono, &empty);
        // start_lo (100) < pre_window (256) but start_hi (3000) >= pre_window ⇒ pre bound non-uniform ⇒ None.
        assert!(!anchor_bracket_matchability_doomed(
            &templates, gap_frames, pre_window, post_window, 100, 3000, &params,
        ));
    }
