...
/// Build run-constant seam-gate config from a repair request (same as production).
pub fn seam_gate_config_from_repair(
    request: &crate::application::PatchAudioRequest,
    sample_rate: u32,
    channels: usize,
    silence_peak_fraction: f32,
) -> SeamGateConfig {
    SeamGateConfig::from_repair(request, sample_rate, channels, silence_peak_fraction)
}
