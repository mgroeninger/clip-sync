...
//!
//! Module map (M-MOD `gap_fingerprint` split): [`schema`] owns the serde corpus / per-gap types
//! and the [`source_id`] identity digest; this facade re-exports them (`pub use schema::*`) and
//! still holds the Spec↔Fingerprint projection and the PCM measurement/builder path pending their
//! own `project` / `measure` slices. See `docs/dev/TEMP-gap-fingerprint-module-split-plan.md`.
