...
fn format_human(
    ...
    if !query_mode {
        if let Some(drift) = align.offset_drift_secs {
            if !align.offsets_consistent {
                out.push_str(&format!("Drift:     end − start = {drift:+.3}s\n"));
                if align.recommended_offset_secs.is_some() {
                    out.push_str(
        ...
        if show_diagnostics {
            if let Some(anchor) = align.end_clip_anchor {
                out.push_str(&format!("{}\n", format_end_clip_anchor_line(anchor)));
            }
            if let Some(end) = align
                .clips
                .iter()
    ...
    if let Some(refine) = &align.high_rate_refinement {
    ...
    if let Some(verify) = &align.offset_verification {
...
struct GapScanJson {
    video_a: PathBuf,
    video_b: PathBuf,
    track_compatibility: Option<TrackCompatibility>,
    ...
    /// scan did not populate it. See `docs/dev/gap-vocabulary.md` § Silence-character pre-gate.
    ...
    /// Normalized amplitude in `[0, 1)` (default ≈ `0.001007`).
...
impl GapScanJson {
    fn from_parts(report: &GapReport, alignment_detail: &clip_sync::AlignmentResult) -> Self {
        Self {
            ...
            min_gap_ms: report.recipe.min_gap_ms(),
            silence_hold_ms: report.recipe.silence_hold_ms(),
            scan_block_ms: report.recipe.scan_block_ms(),
            silence_peak_fraction: report.recipe.silence_peak_fraction(),
            absolute_silence_rms: report.recipe.absolute_silence_rms(),
